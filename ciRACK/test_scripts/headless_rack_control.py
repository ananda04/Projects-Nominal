import sys
sys.dont_write_bytecode = True  # never cache .pyc for local modules (this file, headless_tests/*, shared_control/*) -- always run current source
...
from shared_control import doDriveControl
...
#
# TEST_MODULES' order is the run order: safe_to_test first (a relay-safety
# check, run before anything else touches the rack -- NOTE this only
# monitors during ITS OWN slot; since tests never interleave, it stops
# once do_drive's slot begins), then do_drive (a bounded one-shot test),
# then the remaining continuous tests in this order -- same relative order
...
# ============================================================================
TEST_MODULES = [safe_to_test, do_drive, di_raster_scan,
                ain_ao_loopback, ain_ao_route, fgen_sweep, psu_control]
TEST_MODULE_BY_ID = {m.TEST_ID: m for m in TEST_MODULES}

# "psu" is its own driver bucket, distinct from keysight_34980a/labjack/
# ni_daqmx -- psu_control.py opens its own dedicated BK9115/N5745A/eLoad
# VISA sessions and never touches the shared 34980A daq/inst this
# orchestrator hands every test.
ALL_DRIVERS = ["keysight_34980a", "labjack", "ni_daqmx", "psu"]
